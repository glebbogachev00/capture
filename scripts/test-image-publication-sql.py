...
with tempfile.TemporaryDirectory(prefix='capture-images-pg-', dir='/tmp') as d:
    ...
    try:
        ...
        ok(f"""create role anon; create role authenticated;
        ...
        denied(user(f"insert into storage.objects(bucket_id,name) values ('capture-image-candidates','{A}/{uuid.uuid4()}');"), 'row-level security')
        assert ok(user("select count(*) from storage.objects where bucket_id='capture-image-candidates';", operation='object.get_authenticated')) == '0'
        assert ok(user("select count(*) from storage.objects where bucket_id='capture-images';", operation='object.get_authenticated')) == '0'
