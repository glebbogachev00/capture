#!/usr/bin/env node
/**
 * Probe: the DISTILL settler — does a finished conversation file correctly?
 *
 ...
 *   hasDue         the conversation names a deadline and it must be read
 *   notRouteTo     this thread is the trap; filing there is the failure
...
const CASES_JSON = fileURLToPath(
  new URL("../src/lib/settleCases.json", import.meta.url)
);
...
/* One board for every case, so "routes into the right thread" is a real
   question rather than a hypothetical. */
const THREADS = [
  { id: "t-cap", name: "Capture", about: "building the capture app: bugs, features, decisions" },
  { id: "t-esp", name: "Espresso setup", about: "choosing a machine and dialling in shots" },
  { id: "t-gro", name: "Groceries", about: "things to restock at home" },
];
...
async function settleOnce(turns, threads) {
  const headers = { "Content-Type": "application/json" };
  const cookie = await sessionCookie();
  if (cookie) headers.Cookie = cookie;
  let res = await fetch(`${BASE}/api/distill`, {
    method: "POST",
    headers,
    body: JSON.stringify({ op: "settle", turns, threads }),
  });
  if (res.status === 401 && cookie) {
    ...
    res = await fetch(`${BASE}/api/distill`, {
      method: "POST",
      headers,
      body: JSON.stringify({ op: "settle", turns, threads }),
    });
  ...
  if (res.status >= 500) {
    /* Every tier spent or hiccupping. That is the chain being tired, not the
       engine being wrong — wait and give it one more go before calling it a
       failure, or a long probe run reports its own rate limits as defects. */
    await sleep(6000);
    res = await fetch(`${BASE}/api/distill`, {
      method: "POST",
      headers,
...
function judge(tc, out) {
  ...
  if (e.notRouteTo && out.threadId === e.notRouteTo)
    problems.push(`filed into ${e.notRouteTo} on a shared word, not the subject`);
...
let passed = 0;
const failures = [];
console.log(`settle cases → ${BASE} (${CASES.length} cases)\n`);
for (const tc of CASES) {
  process.stdout.write(`  ${tc.id.padEnd(24)}`);
  try {
    const out = await settleOnce(tc.turns, THREADS);
