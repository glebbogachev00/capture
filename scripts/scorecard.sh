...
echo "lib seams : $(ls src/lib/{adopt,undoOps,tangleOps,intentionOps,tidyPanel,settle}.ts 2>/dev/null | wc -l | tr -d ' ')/6 present"
echo
...
c "$(grep -q 'in-flight push' src/lib/twoDevice.test.ts 2>/dev/null && echo x || echo ' ')" "5 mid-push edit drains to hub (tested)"
c "$(grep -rq 'stale summary' src/lib/*.test.ts 2>/dev/null && echo x || echo ' ')" "6 stale summary cannot overwrite newer content (tested)"
