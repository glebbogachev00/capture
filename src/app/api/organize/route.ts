...
/* One pass per request. Every other route here is capped at 60 seconds
   because that is the platform's ceiling; an earlier version paced four
   passes inside one request, ran for eighty-odd seconds, and was killed in
   production every time while working on a developer machine. The pacing
   lives on the client now, which has no such limit. */
    ...
    z.object({
      ...
      frags: z.array(
        z.object({
          ...
          text: z.string(),
          at: z.number(),
          /* Rides through so the prompt can mark [resolved] and the mapper
             can refuse re-claims — z.object strips unknown keys. */
...
- looks_done — a note that ASKED for something, when the board itself says it happened. The person is offered a "resolved" label; nothing is ever labeled for them, and the note never leaves its thread. Two kinds of evidence count, and only these:
  1. A LATER note in their own words saying the thing shipped, was sent, was fixed, was decided.
  2. The "Ticked off" list — those are receipts, tasks the person finished by their own hand. A note that asked for something on that list IS resolved, and you should say so. This is the one place where the words matching is exactly the point: the receipt quotes the task, so a ticked-off entry that restates the note is proof, not phrase-matching. Always check every un-resolved note against the Ticked off list before answering. sourceId = the thread holding the asking note, sourceFragId = that note, targetId = the thread whose later note says it happened (often the same thread), reason = a plain sentence pointing at the later note ("a later note says the undo button is in and working"). Never claim a note already marked [resolved]. Only when the WHOLE note is answered: a note that asks for three things of which one happened is NOT resolved — say nothing, or propose extract_action for the parts that are still tasks. What you know about the world, or the note being old, are NOT evidence — when the completion is not in their own words on this board, there is no claim.
