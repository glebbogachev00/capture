...
describe("explicit tasks survive the visible board and reload", () => {
  it.each([
    ...
    await waitFor(() => expect(reloaded.container.querySelector("textarea")).not.toBeNull());
    if (actions.length) {
      fireEvent.click(reloaded.getByRole("button", { name: /^Actions/i }));
      ...
      fireEvent.click(reloaded.getByRole("button", { name: /^Threads/i }));
