-- Capture Cloud billing state. Apply after 20260911120000_capture_boards.sql.
-- One user may legitimately have more than one Polar subscription. Access is
-- granted while any subscription remains entitled; each webhook updates only
...
create index if not exists capture_cloud_subscriptions_user_id_idx
  on public.capture_cloud_subscriptions (user_id);
create index if not exists capture_cloud_subscriptions_entitlement_idx
  on public.capture_cloud_subscriptions (user_id, is_entitled, access_expires_at);

alter table public.capture_cloud_subscriptions enable row level security;
revoke all on table public.capture_cloud_subscriptions from anon, authenticated;
...
drop policy if exists capture_cloud_subscriptions_select on public.capture_cloud_subscriptions;
create policy capture_cloud_subscriptions_select on public.capture_cloud_subscriptions
