-- Additive amendment: acknowledge signed events for missing/deleted auth users.
-- Ordering and existing paid-access policy are intentionally unchanged.
...
begin
  ...
  -- Lock a real owner through the transaction so concurrent account deletion
  -- cannot turn the ledger/subscription inserts into a foreign-key retry loop.
  perform 1 from auth.users where id = p_user_id for key share;
  if not found then
    -- Deleted/unrecognized identities cannot receive access. Keep the delivery
    -- for deduplication, without a dangling auth reference or new auth account.
    insert into public.polar_webhook_events (
      event_id, event_type, user_id, polar_subscription_id, event_created_at
    ) values (
      p_event_id, p_event_type, null, p_polar_subscription_id, p_event_created_at
