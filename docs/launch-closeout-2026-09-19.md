...
- The source suite was green before closeout edits: 186 files / 1,748 tests. The combined release candidate passes 195 files / 1,884 tests, including the launch, OpenRouter/Jev, and Search-that-answers lanes plus the final minimal answer-surface and navigation-deduplication regressions.
...
| Preserve every explicit task | Implemented in closeout | Red prompt contract on current main, then 2 files / 8 tests green after three prompt-line changes | Retain this closeout diff; real-provider compliance remains manual. |
| Failed-provider capture preservation | Implemented | Failed-sort settlement stores unsorted input; provider retries are disabled per call; source tests cover recovery | Live provider-dead browser check remains manual. |
...
| Cloud board ownership and tenant derivation | Implemented in source | Verified-claim identity, `X-Capture-Owner`, per-owner repository access, fail-closed boundaries, account-isolation tests | Real two-account deployed isolation remains manual. |
...
| Polar checkout, portal, signed webhooks, lifecycle | Implemented locally | Safe continuation, server-owned plan, signed webhook processing, durable equal-timestamp reconciliation, past-due anchor, disposable PostgreSQL tests | Remote migrations, live Polar binding, payment, cancellation, expiry, retry, and endpoint status remain manual. |
...
| Cloud image validation and tenant paths | Implemented locally | MIME/signature/size checks, paid entitlement, owner precondition, no-store responses | Hosted Storage/RLS behavior remains manual. |
| Durable immutable image publication | Partial, safely default-closed | Fresh UUID candidates, digest-bound publication, unique-key arbitration, stable-byte validation, mode/bucket RPC gate | No activation until approved hosted namespace provenance and five race rounds pass. Legacy cutover remains blocked on provider drain and preservation evidence. |
| Account erasure | Absent end to end; source implementation blocked | Documentation specifies an operator workflow, but no durable deletion fence, recent-auth contract, provider drain, complete billing behavior, privileged retry identity, or hosted readback proof exists | Do not improvise. Exact unblockers are recorded below. |
...
| Cloud value metrics and per-user quotas | Absent / partial | Playground has bounded aggregate usage events; model limiter is per-IP/in-memory; no full Cloud metric set or server-side per-user model quota | Do not add analytics during closeout without a privacy and operations decision. |
...
## Account erasure decision: do not add the route yet

The complete fail-closed path is **not unambiguous enough to implement safely**. A route that merely deletes rows, Storage objects, then the Auth user would create false assurance and can race admitted Storage completions or billing regrant. The minimum unblockers are:

1. **Recent owner authentication:** decide and specify what “verify the current session again” means for a destructive request (fresh OTP/challenge, bounded `auth_time`, or another provider-supported proof). Current verified claims plus `X-Capture-Owner` are strict tenant binding, but not proof of a recent destructive-action confirmation.
2. **Durable deletion fence and retry identity:** approve a non-cascading erasure operation/tombstone that board writes, image admissions/publications, checkout, webhook apply, and reconciliation all check before any cleanup starts. Define how an interrupted operation resumes after sessions are revoked or the Auth user is gone, without allowing cross-tenant lookup or deletion.
3. **Billing disposition:** decide whether erasure cancels active Polar subscriptions/customers, when cancellation occurs, and how signed late webhooks are acknowledged without recreating access or continuing charges. Revoking the local entitlement row alone is insufficient.
4. **Provider drain evidence:** obtain a Supabase-documented and hosted-verified procedure covering already-admitted standard, resumable, and signed uploads plus finalization/cleanup. Current RLS closes new admissions but cannot prove no privileged completion appears after Storage readback.
5. **Privileged hosted boundary:** authorize the trusted backend host and provision its existing Supabase secret credential plus Polar authority through the normal secret workflow. Approve the exact bucket inventory (legacy, active candidate bucket, abandoned candidates, and any other user-owned bucket) and bounded pagination/retry behavior. No secret value is needed in source or chat.
6. **Completion/readback contract:** approve success only after Storage API deletion, independent owned-object inventory readback, Auth admin deletion/readback, app-row cascade verification, and billing-state verification. Define the minimal content-free audit receipt and its retention.

No erasure route, migration, UI, or privileged dependency was added in this pass. Existing SQL tests prove only local FK/RLS behavior; they cannot prove backend-byte deletion or a hosted provider drain.
...
3. Verify OTP delivery and fresh sign-in, logout, expiry, cross-tab/account-switch revocation, same-account reverification, two-account board/image isolation, and anonymous/free/paid denials.
4. Run sandbox checkout, signed webhook, entitlement, portal, scheduled cancellation, past-due anchor, expiry/revocation, duplicate delivery, and reconciliation retry. Confirm provider spend ceiling and test-instrument scope.
...
6. With approved non-sensitive text, verify every explicit task survives the real provider chain and the forced-dead path preserves an honest unsorted capture.
...
1. On clean desktop and phone profiles, reach the first useful sorted result, understand Undo/correction, traverse public routes/navigation, inspect console/network, and exhaust the public trial intentionally.
...
- Real-provider compliance for the explicit-task prompt is unknown.

## Executed verification

- `npm run check:launch-hosted`: **20 files / 245 tests passed**; disposable billing SQL passed; blocked-legacy and fresh-publication SQL passed; the five-round browser simulator passed its success case and rejected both-success, missing-no-store, denied, and read-error cases as expected.
- Combined release-candidate `npm run check`: passed after bounding Vitest to one worker.
  - ESLint: zero errors, zero warnings.
  - Vitest: **195 files / 1,884 tests passed**.
